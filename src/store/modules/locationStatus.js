...
const BASE_URL = "/api/v1/location-statuses";

export default {
  namespaced: true,
  state: {
    locationStatuses: [],
    isLocationStatusLoading: false,
  },
  mutations: {
    SET_LOCATION_STATUSES: (state, payload) => {
      state.locationStatuses = payload;
    },
    SET_LOCATION_STATUSES_LOADING: (state, payload) => {
      state.isLocationStatusLoading = payload;
    },
  },
  getters: {
    locationStatuses: (state) => state.locationStatuses,
    isLocationStatusLoading: (state) => state.isLocationStatusLoading,
  },
  actions: {
    ...
    getItems: async ({ commit }) => {
      try {
        commit("SET_LOCATION_STATUSES_LOADING", true);
        const { data } = await httpRequest.getRequest(`${BASE_URL}`);
        console.log(data);
        commit("SET_LOCATION_STATUSES", data);
      } catch (error) {
        commit("SET_LOCATION_STATUSES", []);
        console.log(error);
      } finally {
        commit("SET_LOCATION_STATUSES_LOADING", false);
      }
    },
