    ...
    >
      <template slot="searchButton">
        <BaseAcceptButton
          small
          @click="openDialog"
          label="Crear nueva cantidad de muestra"
          v-if="canCreate"
        />
      </template>
    </BaseDatatable>
...
<script>
import { SampleQuantityHeaders } from "../../helpers/headersDatatable";
...
import SampleQuantity from "../../models/SampleQuantity";
...
export default {
  ...
  data: () => ({
    dialog: false,
    editedItem: new SampleQuantity(),
    editedIndex: -1,
    defaultItem: new SampleQuantity(),
    snackbar: false,
    headers: SampleQuantityHeaders,
    dialogDelete: false,
  ...
  computed: {
    ...mapGetters({
      sampleQuantities: "sampleQuantity/sampleQuantities",
      namedPermissions: "auth/namedPermissions",
    }),

    items() {
      if (!this.sampleQuantities) return [];
      return this.sampleQuantities.collection;
    },

    formTitle() {
      return this.editedIndex === -1
        ? "Crear cantidad de muestra"
    ...
    canCreate() {
      ...
      return this.namedPermissions.includes("sampleQuantity.create");
    },

    canUpdate() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("sampleQuantity.update");
    },

    canDelete() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("sampleQuantity.delete");
    },

    canShow() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("sampleQuantity.show");
    },
  },

  methods: {
    ...mapActions({
      index: "sampleQuantity/getItems",
      indexPaginate: "sampleQuantity/getPaginatedItems",
      store: "sampleQuantity/postItem",
      update: "sampleQuantity/putItem",
      delete: "sampleQuantity/deleteItem",
      show: "sampleQuantity/showItem",
      changeStatus: "sampleQuantity/changeStatusItem",
    }),
