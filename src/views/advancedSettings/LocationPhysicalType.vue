<template>
  <v-container>
    <BaseHeaderModule
      title="Módulo de tipos de ubicación física"
      subtitle="En este módulo podrás gestionar los tipos de ubicación física."
    />
    ...
    >
      <template slot="searchButton">
        <BaseAcceptButton
          small
          @click="openDialog"
          label="Crear nueva ubicación física"
          v-if="canCreate"
        />
      </template>
    </BaseDatatable>
...
<script>
import { LocationPhysicalTypeHeaders } from "../../helpers/headersDatatable";
...
import LocationPhysicalType from "../../models/LocationPhysicalType";
...
export default {
  ...
  data: () => ({
    dialog: false,
    editedItem: new LocationPhysicalType(),
    editedIndex: -1,
    defaultItem: new LocationPhysicalType(),
    snackbar: false,
    headers: LocationPhysicalTypeHeaders,
    dialogDelete: false,
  ...
  computed: {
    ...mapGetters({
      locationPhysicalTypes: "locationPhysicalType/locationPhysicalTypes",
      namedPermissions: "auth/namedPermissions",
    }),

    items() {
      if (!this.locationPhysicalTypes) return [];
      return this.locationPhysicalTypes.collection;
    },

    formTitle() {
      return this.editedIndex === -1
        ? "Crear tipo de ubicación física"
        : "Editar tipo de ubicación física";
    },
    ...
    canCreate() {
      ...
      return this.namedPermissions.includes("locationPhysicalType.create");
    },

    canUpdate() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("locationPhysicalType.update");
    },

    canDelete() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("locationPhysicalType.delete");
    },

    canShow() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("locationPhysicalType.show");
    },
  },

  methods: {
    ...mapActions({
      index: "locationPhysicalType/getItems",
      indexPaginate: "locationPhysicalType/getPaginatedItems",
      store: "locationPhysicalType/postItem",
      update: "locationPhysicalType/putItem",
      delete: "locationPhysicalType/deleteItem",
      show: "locationPhysicalType/showItem",
      changeStatus: "locationPhysicalType/changeStatusItem",
    }),
