<template>
  <v-container>
    <BaseHeaderModule
      title="Módulo de intenciones de examen"
      subtitle="En este módulo podrás gestionar las intenciones de examen."
    />
...
<script>
import { ServiceRequestIntentHeaders } from "../../helpers/headersDatatable";
...
import ServiceRequestIntent from "../../models/ServiceRequestIntent";
...
export default {
  ...
  data: () => ({
    dialog: false,
    editedItem: new ServiceRequestIntent(),
    editedIndex: -1,
    defaultItem: new ServiceRequestIntent(),
    snackbar: false,
    headers: ServiceRequestIntentHeaders,
    dialogDelete: false,
  ...
  computed: {
    ...mapGetters({
      serviceRequestIntents: "serviceRequestIntent/serviceRequestIntents",
      namedPermissions: "auth/namedPermissions",
    }),

    items() {
      if (!this.serviceRequestIntents) return [];
      return this.serviceRequestIntents.collection;
    },

    formTitle() {
      return this.editedIndex === -1
        ? "Crear intención de examen"
        : "Editar intencion de examen";
    },
    ...
    canCreate() {
      ...
      return this.namedPermissions.includes("serviceRequestIntent.create");
    },

    canUpdate() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("serviceRequestIntent.update");
    },

    canDelete() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("serviceRequestIntent.delete");
    },

    canShow() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("serviceRequestIntent.show");
    },
  },

  methods: {
    ...mapActions({
      index: "serviceRequestIntent/getItems",
      indexPaginate: "serviceRequestIntent/getPaginatedItems",
      store: "serviceRequestIntent/postItem",
      update: "serviceRequestIntent/putItem",
      delete: "serviceRequestIntent/deleteItem",
