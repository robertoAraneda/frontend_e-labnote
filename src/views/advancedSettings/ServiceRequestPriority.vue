<template>
  <v-container>
    <BaseHeaderModule
      title="Módulo de prioridades de solicitud de examen"
      subtitle="En este módulo podrás gestionar las prioridades de solicitud de examen."
    />
...
export default {
  ...
  data: () => ({
    dialog: false,
    editedItem: new ServiceRequestPriority(),
    editedIndex: -1,
    defaultItem: new ServiceRequestPriority(),
    snackbar: false,
    headers: ServiceRequestPriorityHeaders,
    dialogDelete: false,
  ...
  computed: {
    ...
    items() {
      if (!this.serviceRequestPriorities) return [];
      return this.serviceRequestPriorities.collection;
    },

    formTitle() {
      return this.editedIndex === -1
        ? "Crear prioridad de solicitud de examen"
        : "Editar prioridad de solicitud de examen";
    },
    ...
    canCreate() {
      ...
      return this.namedPermissions.includes("serviceRequestPriority.create");
    },

    canUpdate() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("serviceRequestPriority.update");
    },

    canDelete() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("serviceRequestPriority.delete");
    },

    canShow() {
      if (!this.namedPermissions) return false;
      return this.namedPermissions.includes("serviceRequestPriority.show");
    },
  },

  methods: {
    ...mapActions({
      index: "serviceRequestPriority/getItems",
      indexPaginate: "serviceRequestPriority/getPaginatedItems",
      store: "serviceRequestPriority/postItem",
      update: "serviceRequestPriority/putItem",
      delete: "serviceRequestPriority/deleteItem",
